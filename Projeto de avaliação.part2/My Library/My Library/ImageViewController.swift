...
class ImageViewController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
   ...
   // Segunda tela 2 labels e 1 imageView
    ...
    @IBAction func saveButton(_ sender: Any) {
     //   selectImage?.append(coverOutlet.description)

        coverOutlet.image = UIImage(named: "Goya")
      // selectImage = "Goya"
//        coverOutlet.image = UIImage(named: "\(selectImage?.description)")
        print(selectImage?.description)
    }
    ...
    override func prepare(for segue: UIStoryboardSegue,
       sender: Any?) {
        ...
        guard segue.identifier == "alterar" else { return }
        
        let image = "Bosch"
        print(image)
        
        selectImage = book?.image
        print(selectImage?.description)
